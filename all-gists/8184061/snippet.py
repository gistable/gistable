...
__pver__     = '0.5.1'
__pdate__    = '2009-01-21'
__pauthor__  = "Giampaolo Rodola' <g.rodola@gmail.com>"
__pweb__     = 'http://code.google.com/p/pyftpdlib/'

__ver__      = '1.0a0'
__date__     = '2009-03-25'
__author__   = 'd13 <dainan@staff.sina.com.cn>'
__web__      = 'http://www.dn13.org/'
...
if __name__ == "__main__":
    ...
    if '-h' in opts :
        print """
pyftp        : single file ftp server write in python. implement on pyftpdlib.
...
implement on :
  ...
  version: %(__pver__)s
     date: %(__pdate__)s
   author: %(__pauthor__)s
      web: %(__pweb__)s

usages       :
-p  <port>     use port
-d  <path>     set the path as ftp root path. default is './'
-h             see help
""" % vars()
        sys.exit(1)
...
#    authorizer.add_anonymous( opts.get( '-d', os.getcwd() ), perm='elradfmw' )
    authorizer.add_user('username', 'xxxx', opts.get( '-d', os.getcwd() ), perm='elradfmw' )
    
    FTPHandler.authorizer = authorizer
    
    address = ( '', int( opts.get('-p',21) ) )
